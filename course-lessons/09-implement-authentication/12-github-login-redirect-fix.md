...
After some research, I found the `signIn` function we used with OAuth is not reliable for redirects with v5. The solution is to instead, use the `signIn` from `@/auth` in a sever action.
